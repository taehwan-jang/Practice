	...
	public static void main(String[] args) {
		...
		try {
			...
			if(con!=null) {
				...
				/*
				1.쿼리문 작성 
					: 쿼리문을 작성할때 주의할 점은 줄바꿈을 할 때 앞뒤로
					스페이스를 삽입하는 것이 좋다. 그렇지 않으면 서로 문장이
					이어지게되어 Syntax Error가 발생하게 된다.
				 */
				String sql = "SELECT * FROM employees WHERE "
						+ " department_id=50 "
						+ " ORDER BY employee_id DESC ";
				/*
				2.쿼리문 전송을 위해 Statement인터페이스를 통한 객체를
				생성한다. 주로 정적쿼리문을 실행할 때 사용한다. 
				 */
				Statement stmt = con.createStatement();
				/*
				3.쿼리문을 전송한 후 실행한 결과를 ResultSet객체를 통해
				반환받는다. 해당 객체는 select쿼리를 실행했을때만 사용한다.
				 */
				ResultSet rs = stmt.executeQuery(sql);
				/*
				4.반환된 결과를 갯수만큼 반복한다. next()메소드는
				출력할 레코드가 있는지 확인할 때 사용한다. 만약 더이상
				출력할 레코드가 없다면 false를 반환한다.
				 */
				while(rs.next()) {
					/*
					get~~~() 메소드를 통해 각 컬럼에 접근한다. 오라클의
					자료형은 숫자, 문자, 날짜 3가지 형태이므로 각 메소드도
					그와 동일하다. 각 컬럼에 접근시 인덱스, 컬럼명 두가지
					모두 사용할 수 있다.
					 */
					//컬럼명을 직접 써도 되고 인덱스를 써도 된다 제일 좋은건 
					//컬럼명을 직접 쓰는거임. 인덱스는 이건 SELECT * FROM일때 편함ㅋ
