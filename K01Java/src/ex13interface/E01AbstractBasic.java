...
abstract class AbsClass1 {
	...
	public static final int MIN_DOUBLE = Double.MIN_EXPONENT;
...
class AbsClassChild1 extends AbsClass1 {
	...
	void newMethod() {}
	/*
	추상메소드를 포함하지 않은 추상클래스를 상속하면
	오버라이딩은 강제가 아닌 선택사항이다. 즉 필요한
	경우에만 오버라이딩한다.
	 */
	@Override
	void instanceMethod() {//AbsClass1에 abstract 메소드가 없으니 
		System.out.println("필요없으면 오버라이딩 안해도됨");
	}
}
...
//		추상클래스는 객체를 생성할 수 없다.
//		AbsClass1 absClass1 = new AbsClass1();//abstract class이므로 객체생성불가
		
//		추상클래스를 상속한 하위클래스는 객체생성이 가능하다. 단 하위클래스가 추상이 아닐때!!
		...
		((AbsClassChild2)absClass2).newMethod();/*
			부모타입의 참조변수로 자식객체에 접근하려면 다운캐스팅후
			접근해야 한다.
		*/
