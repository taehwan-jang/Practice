...
/*
 컴퓨터를 표현 : 기본적인 컴퓨틴 환경을 제공하여 입력된 내용을
 	계산한다. 
 */
class Computer {
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		...
		인자생성자를 정의해, 기본생성자는 생성되지 않기 떄문이다.
		 */
		super(name);
		battary = initCharge;
	}
	...
	public void movigCal() {
		...
		/*
		private으로 선언된 멤버메서도 이믈 직접 호출은 불가능하다.
		 */
//		keboardTouch();
//		claculate();
		calculatorFunc();
...
class TabletNotebook extends NotebookComputer {
	...
	public void write(String penInfo) {
		...
		/*
		A.compareTo(B) 
			: 문자열 A와 B를 비교하여 같은 문자열로 판단되면
			0이 반환하고 서로 다른 문자열로 판단하면 -1, 1을 
			반환하는 String클래스에 정의된 메소드.
		 */
