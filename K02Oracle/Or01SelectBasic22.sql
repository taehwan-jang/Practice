/**********************************
파일명 : OrSelcetBasic.sql
처음으로 실행해보는 질이의(SQL문 혹은 Query문)
...
형식]
    ...
    order by 정렬할컬럼 asc(오름차순), dese(내림차순);
*/

--사원테이블에 저장된 모든 레코드 조회하기
select * from employees;

--컬럼명을 지정해서 개발자가 보고싶은 컬럼만
--사원번호, 이름, 이메일, 부서아이디만 조회
...
--오라클은 기본적으로 대소문자를 구분하지 않는다. 예약어는 둘 다 사용 가능하다
select employee_id "사원아이디", first_name, last_name "성"
from EMPLOYEES where first_name='William';

--단 레코드인 경우는 대소문자를 구분한다. 아래 SQL문을 실행하면 결과가 나오지 않는다.
select employee_id "사원아이디", first_name, last_name "성"
from EMPLOYEES where first_name='WILLIAM';--이름 전체가 대문자로 작성됨.

/*
where절을 이용해서 조건에 맞는 레코드 추출하기
    : last_name이 smith인 레코드를 가져온다.
    주의] where절에 조건을 입력할때 컬럼이 문자형이면
    싱글쿼테이션을 사용해야한다. 숫자형인 경우 생략가능하다.
*/
...
/*
입사일이 04년 01월 01일 이후인 사원정보를 인출하시오
...
/*
and 연산자 : 둘 이상의 조건이 동시에 만족할때 레코드를 가저온다.
    부서아이디가 50이면서 매니저아이디가 100인 사원정보를 인출하시오
*/
SELECT--select 자동완성하면 이런 폼으로 나옴
    *
FROM employees where department_id=50 and manager_id=100;


/*
 in 연산자 : or 연산자와 비슷한 기능으로 하나의 칼럼에 여러값으로
    조건을 걸고 싶을때 사용하는 연산자
    급여가 4200, 6400, 8000인 직원의 정보를 조회하시오.
*/
--방법1 : or를 사용한다. 컬럼명이 반복적으로 기술되어야 한다
SELECT
    *
FROM employees where salary=4200 or salary=6400 or salary=8000;

--방법2 : in 연산자를 사용한다. 컬럼명을 한번만 기술하면 된다
SELECT
    *
...
/*
 between and 연산자 : 칼럼의 구간을 정해 검색할 때 사용한다.
    급여가 4000~8000 사이의 사원장보를 조회하시오.
*/
--방법1
SELECT
    *
...
/*
 distinct 연산자 : 컬럼에서 중복되는 레코드를 제거할때 사용한다.
    특정 조건으로 select했을때 하나의 칼럼에서 중복되는 값이
...
/*
 like 연산자 : 특정 키워드를 통한 문자열 검색하기
    형식] 컬럼명 like '%키워드%' ///// 컬럼명 like '_키워드_'
    와일드카드 사용법
        % : 모든 문자 혹은 문자열을 대체한다. 
        ex) D로 시작하는 단어 : D% -> Da, Dream, Drama...
              Z로 끝나는 단어 : %Z -> aZ, abcZ ...
              C가 포함되는 단어 : %C% -> aCb, abCd, Vitamin-C
              
         _ : 언더바는 하나의 문자를 대체한다.
         ex) D로 시작하는 3글자의 단어 : D__ -> Dab, Dad, Dxy
               A가 중간에 들어가는 3글자의 단어 : _A_ ->aAa, zAd...
*/
...
--phone_number에 1344가 포함되는 모든 직원을 검색하시오
SELECT
    *
...
/* 
 레코드 정렬허가(Sorting)
    오름차순정렬 : order by 컬럼명 asc(생략가능 [디폴트])
...
/*
사원정보 테이블에서 급여가 낮은순서에서 높은순서로 나오도록 정렬하여 조히하시오.
