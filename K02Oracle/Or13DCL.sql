...
--cmd : no privileges on tablespace 'SYSTEM' 에러
--테이블생성 권한 부여-오류발생되었음(재확인) 
--테이블 스페이스가 없어서 생성되지 않는다.
...
테이블 스페이스란?
    ...
    hr계정의 데이터를 저장하는 users라는 테이블 스페이스가 자동으로 
    생성된다.
*/
--테이블 스페이스 조회하기
...
--테이블 생성 시도1 -> insufficient privileges 권한불충분
--테이블 생성 권한 추가 
grant create table to test_user2;
--테이블 생성 시도2 -> no privileges on tablespace 'USERS' 테이블스페이스에
--권한 없음
...
생성된 사용자의 디폴트 테이블스페이스를 변경하고 싶은 경우
    : test_user1의 디폴트 테이블스페이스를 users로 변경한다.
*/
...
--시스템 권한 목록 확인하기 :  208개의 시스템권한이 있음
...
/*
4] ROLE을 통한 여러가지 권한 동시에 부여하기
    : 여러 사용자가 다양한 권한을 효과적으로 관리할 수 
    있도록 관련된 권한끼리 묶어놓은 것을 말한다. 
※ 우리는 실습을 위해 새롭게 생성한 계정에 connect, resource를 주로 부여한다.    
*/
--test_user2 사용자에 아래 두가지 롤을 부여한다.
grant connect, resource to test_user2;
--cmd에서 new_table.sql파일을 실행한다.(시퀀스 생성, 레코드 입력)
