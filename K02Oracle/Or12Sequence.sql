...
/*
시퀀스
    : 테이블의 컬럼에 중복되지 않는 순차적인 일련번호를 부여하는
...
insert into tb_goods values (1, '감자깡');--입력실패
--시퀀스가 없다면 max() 함수를 통해 일련번호를 구해야 한다.
select max(g_idx)+1 from tb_goods;
insert into tb_goods values (2, '감자깡');
/*
입력전 함수를 통해 확인하는 것은 번거로우므로 시퀀스를 생성하여
입력시 사용한다.
*/
create sequence seq_serial_num
    increment by 1--증가치 (한번에 1씩 증가)
...
--10번 이상 반복시 cycle로 PK중복 발생    
insert into tb_goods values (seq_serial_num.nextval, '몽쉘');    
select * from tb_goods;

/*
    시퀀스의 cycle옵션에 의해 최대값에 도달하면 다시 처음으로 돌아가
    일련번호가 생성되므로 무결성 제약조건에 위배된다. 즉 기본키에 상요할
    시퀀슨느 cycle옵션을 사용하면 안된다.
*/
...
/*
동일 쿼리문 내에서는 nextval을 여러번 사용하더라도 항상 같은 값을
반환한다.
*/
...
--인덱스는 수정이 불가능하므로 만약 수정이 필요하다면 삭제후 다시 생성한다.
