...
*/
select trunc(1234.56789, 2) "Trunc 양수인자" from dual;--1234.12
select trunc(1234.56789) "Trunc 인자 없음" from dual;--1234
select trunc(1234.56789,-2) "Trunc음수인자" from dual;--1200
...
SELECT
    *
FROM employees where commission_pct is not null;--1.커미션을 받는 사원들

select first_name, commission_pct, trunc(commission_pct,1) 
from employees where commission_pct is not null;--2.위 결과에서 소수점을 처리후 인출
...
-- 첫 번째 항목 : 소수이하 6자리까지 표현하므로 7을 올림처리
-- 두 번째 항목 : 소수이하 4자리까지 표현하므로 1을 버림처리
select round(0.1234567, 6), round(2.345612, 4) from dual;

/*
mod() : 나머지를 구하는 함수 JAVA에서 %와 같음
*/
select mod(99, 4) "99를 4로 나눈 나머지" from dual;

--숫자가 들어가는 자리에 숫자 형식의 컬럼을 대체할 수 있다.
select first_name, mod(salary, department_id) "급여%부서번호" from employees;

/*
power() : 거듭제곱을 계산할대 사용하는 함수
